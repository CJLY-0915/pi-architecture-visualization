...
- **再次采集后，上一份模型画的关系图会留在屏幕上**。`loadModel` 会重置关系图，`adoptCollectedModel` 不会，于是采集得到 9 个节点的模型时，阅读器显示 9 节点、关系图却仍写着"20 层 · 20 节点"——同一个面板上同时摆着两个模型。现在采集也撤回关系图与漂移结果，与读取文件保持一致。实测：采集 9 节点模型后 `diagram-status` 由 `ready` 变 `idle`、`#diagram-surface` 不再有 `<svg>`。
