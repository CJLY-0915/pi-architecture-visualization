...
面板最小三步：点"采集并生成模型"（无需任何前置模型，也无需先在会话里让 agent 建模）→ 模型当场载入下方阅读器，在节点列表里选一项看详情 → 点"绘制关系图"看分层关系图，点图中任一节点聚焦它的直接邻居。图可以按住拖拽平移、用滚轮或"放大/缩小"按钮缩放（0.25–4 倍，滚轮以指针为锚点），"重置视图"回到初始画面；缩放与平移只是视图状态，每次重绘都会重置，不会改动模型。采集本身不写工作区；模型要长期保留，用在采集结果下方出现的"保存这份模型"——它先展示目标路径、该路径当前是否存在、以及新旧两边的节点/关系/证据数，再让你在三个动作里选："创建"（仅当目标不存在）、"另存为快照"（`architecture/snapshots/<sha256>.json`，内容寻址、永不覆盖）、"覆盖写入"（唯一会替换已有文件的动作）。写入标准路径后，面板立即从磁盘重新载入。已经有模型文件时，展开后面的"读取已有模型"填路径即可。导出预览只是内存文本，可复制到剪贴板，不下载、不写工作区。
...
`node --test tests/*.test.js` 当前 353/353（已在 `architecture/` 缺席的条件下复现 CI 环境验证通过）；三平台 CI 最新三个 run 已回看并三平台全绿——1.5.1 的 run [35953559639](https://github.com/CJLY-0915/pi-architecture-visualization/actions/runs/35953559639)（commit `8b87675`，2026-09-24T03:56:56Z 起）、文档修正的 run [35953927755](https://github.com/CJLY-0915/pi-architecture-visualization/actions/runs/35953927755)（commit `1aa5a1c`，2026-09-24T04:02:19Z 起）与"再次采集撤回关系图"修复的 run（commit `cfecfae`，2026-09-24T04:30:06Z 起），各 3 个 job（ubuntu/windows/macos-latest）、每 job 7 个 step 全部 `success`；读取方式为本机请求 GitHub Actions API，属本地日志而非用户证言。`bb03244` 之后曾因测试读取 gitignore 目录而三平台全红，1.4.1 修复后确认恢复；干净镜像 `PluginCheck` 无错误通过（1.4.0 起运行时集合 41 文件，1.5.0 起 **44 文件**——新增 `src/core/diagram.js`、`src/core/drift.js`、`src/host/drift-check.js`，由 `tests/package-scope.test.js` 断言；44 文件集合的干净镜像已于 1.5.1 实测重跑通过）；`.piplug` 只由不含会话目标文件与临时镜像的干净镜像经官方 `PluginPack` 生成并审计——1.5.1 产物为 `dist/local.architecture-visualization-1.5.1.piplug`（481,581 字节，SHA-256 `025186b7…14221ff3`，包内 44 条目全部未压缩存储、路径相对、无穿越，与镜像逐字节一致），另做过解包后的宿主桩冒烟。逐项宿主验收（A 组生命周期 15 项、B 组场景技能）见 [docs/host-acceptance.md](./docs/host-acceptance.md)。
