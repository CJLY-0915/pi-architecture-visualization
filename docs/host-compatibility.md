...
5. 已生成 1.1.0 的 `.piplug`；A10 的安装回归是针对 0.1.0 做的，1.0.0/1.1.0 新包需在干净环境重做安装→启用→升级→禁用→卸载，之后才可将分发包视为宿主 E2E 通过。
