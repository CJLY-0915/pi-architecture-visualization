...
退出标准：默认视图不变（`max-width: 100%` 保留，宽画布仍整体可见），缩放与平移叠加在它之上；缩放到达上下限即停；重绘不继承旧偏移；三个按钮在无图时隐藏。运行时集合仍 44 文件，测试 350 → 351。真机确认仍记在 A16（本次只在本机 Chromium 用桥接桩预览页验证过，未在宿主面板里拖过）。
