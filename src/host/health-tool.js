...
async function executeHealth(host, request) {
  try {
    const tool = definition();
    if (!tool) return fail('unsupported_input', 'Architecture health is not declared.');
    if (!request || typeof request !== 'object' || Array.isArray(request) || typeof request.path !== 'string' || request.path.length === 0 || request.path.length > 1024 || Object.keys(request).some((key) => key !== 'path')) {
    ...
    const loaded = await readModel(host, request.path, { allowInvalidModel: true });
